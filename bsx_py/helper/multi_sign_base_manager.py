...
class MultiSigBaseManager(AccountManager):
    def __init__(self, owner_addr: str, wallet_secret: str, signer_secret: str, domain: str,
        ...
        api_key = self._auth_client.register(
            RegisterParams(wallet_addr=owner_addr, wallet_pkey=wallet_secret, signer_pkey=signer_secret, message="")
        )
        self._acc_storage.set_signer_addr(signer.address)
        self._acc_storage.set_signer_pkey(signer.key)
        self._acc_storage.set_wallet_addr(owner_addr)
        self._acc_storage.set_wallet_pkey(wallet_secret)
